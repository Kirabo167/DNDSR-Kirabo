...
namespace DNDS::ACMVariable
{
    ...
    struct KernelConfiguration
    {
        ...
        std::array<real, 5> initialState{1, 1, 0, 0, 0};
        real initialDensityAmplitude = 0; ///< Relative cosine density perturbation; magnitude < 1.
        std::array<real, 3> initialDensityWaveNumber{0, 0, 0}; ///< Radian wave vector; physical coordinates.
        std::array<real, 5> boundaryValue{1, 1, 0, 0, 0};
        std::array<real, 5> leftState{1, 1, 0, 0, 0};
        std::array<real, 5> rightState{2, 2, 0, 0, 0};
        ...
        DNDS_DECLARE_CONFIG(KernelConfiguration)
        {
            ...
            config.field_section(
                ...
                "turbulenceSettings",
                "Segregated variable-density ACM turbulence settings");
            ...
            DNDS_FIELD(initialState, "Uniform initial ACM state [rho,mx,my,mz,p]");
            DNDS_FIELD(initialDensityAmplitude, "Relative cosine density amplitude", DNDS::Config::range(-0.999, 0.999));
            DNDS_FIELD(initialDensityWaveNumber, "Initial density cosine wave vector [kx,ky,kz]");
            DNDS_FIELD(boundaryValue, "Prescribed default boundary state [rho,mx,my,mz,p]");
            DNDS_FIELD(leftState, "Preview left state [rho,mx,my,mz,p]");
            DNDS_FIELD(rightState, "Preview right state [rho,mx,my,mz,p]");
         ...
         * @return Initial state `[rho,mx,my,mz,p]`.
         */
        State InitialState() const;

        /**
         * @brief Convert the configured default boundary value to an ACM state.
         * @return Boundary value `[rho,mx,my,mz,p]`.
         */
